...
public class WeatherDataParser {
    ...
    public static double getMaxTemperatureForDay(String weatherJsonStr, int dayIndex)
        ...
        JSONArray listArray = weatherJsonObject.getJSONArray("list");
        JSONObject dayNode = listArray.getJSONObject(dayIndex);
        ...
        /*
            If we want the max from all max's.
            for (int dayIndex = 0; dayIndex < 7; dayIndex++) {

            JSONArray dtNode = listArray.get[0];
            JSONObject tempNode = dtNode.getJSONArray("list");
            double maxTemp = tempNode.getDouble("max");
            if (maxTemp >= maximumTemp)
                maximumTemp = maxTemp;
        }


        if (maximumTemp == Double.NEGATIVE_INFINITY)
            return -1;
        */
